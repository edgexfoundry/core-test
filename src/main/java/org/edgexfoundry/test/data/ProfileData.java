...
public interface ProfileData {
	...
	static void checkTestData(DeviceProfile p, String id) {
		...
		assertEquals("Device profile origin does not match expected", CommonData.TEST_ORIGIN, p.getOrigin());
		assertEquals("Device profile commands list size does not match expected", 1, p.getCommands().size());
		assertEquals("Device profile description does not match expected", TEST_DESCRIPTION, p.getDescription());
		assertArrayEquals("Device profile labels does not match expected", TEST_LABELS, p.getLabels());
		assertEquals("Device profile manufacturer does not match expected", TEST_MAUFACTURER, p.getManufacturer());
