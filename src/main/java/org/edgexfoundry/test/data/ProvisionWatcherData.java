...
public interface ProvisionWatcherData {
	...
	static void checkTestData(ProvisionWatcher watcher, String id) {
		...
		assertEquals("Watcher's origin does not macth expected origin", CommonData.TEST_ORIGIN, watcher.getOrigin());
		assertEquals("Watcher's identifiers size does not match expected size", 2, watcher.getIdentifiers().size());
		assertEquals("A watcher identifier does not match expected", VAL1, watcher.getIdentifiers().get(KEY1));
		assertEquals("A watcher identifier does not match expected", VAL2, watcher.getIdentifiers().get(KEY2));
