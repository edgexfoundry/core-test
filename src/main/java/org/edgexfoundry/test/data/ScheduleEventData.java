...
public interface ScheduleEventData {
	...
	static ScheduleEvent newTestInstance(){
		...
		ScheduleEvent event = new ScheduleEvent(TEST_SCHEDULE_EVENT_NAME, a, TEST_PARAMS, TEST_SCHEDULE_NAME, TEST_SERVICE_NAME);
	...
	static void checkTestData(ScheduleEvent event, String id) {
		...
		assertEquals("Schedule event name does not match expected", TEST_SCHEDULE_EVENT_NAME, event.getName());
		assertEquals("Schedule event origin does not match expected", CommonData.TEST_ORIGIN, event.getOrigin());
		assertEquals("Schedule event addressable name does not match expected", AddressableData.TEST_ADDR_NAME, event.getAddressable().getName());
		assertEquals("Schedule event service does not match expected", TEST_SERVICE_NAME, event.getService());
		assertEquals("Schedule event params does not match expected", TEST_PARAMS, event.getParameters());
