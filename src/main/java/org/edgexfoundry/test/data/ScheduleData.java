...
public interface ScheduleData {
	...
	static Schedule newTestInstance() {
		...
		Schedule s = new Schedule(TEST_SCHEDULE_NAME, start, end, TEST_FREQUENCY_2S, TEST_CRON, TEST_RUN_ONCE_FALSE);
		s.setOrigin(CommonData.TEST_ORIGIN);
		return s;
	}

	static void checkTestData(Schedule schedule, String id, String start, String end, boolean runonce) {
		...
		assertEquals("Schedule origin does not match expected", CommonData.TEST_ORIGIN, schedule.getOrigin());
		...
		assertEquals("Schedule frequency does not match expected", TEST_FREQUENCY_2S, schedule.getFrequency());
