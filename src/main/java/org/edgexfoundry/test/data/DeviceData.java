...
public interface DeviceData {
	...
	static void checkTestData(Device d, String id) {
		...
		assertEquals("Device addressable does not match expected", TEST_ADDR_NAME, d.getAddressable().getName());
		assertEquals("Device admin state does not match expected", TEST_ADMIN, d.getAdminState());
		assertEquals("Device description does not match expected", TEST_DESCRIPTION, d.getDescription());
		assertArrayEquals("Device labels does not match expected", TEST_LABELS, d.getLabels());
		assertEquals("Device last connected does not match expected", TEST_LAST_CONNECTED, d.getLastConnected());
		assertEquals("Device last reported does not match expected", TEST_LAST_REPORTED, d.getLastReported());
		...
		assertEquals("Device profile does not match expected", ProfileData.TEST_PROFILE_NAME, d.getProfile().getName());
		assertEquals("Device service does not match expected", ServiceData.TEST_SERVICE_NAME, d.getService().getName());
