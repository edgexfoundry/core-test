...
public interface ValueDescriptorData {
	...
	static ValueDescriptor newTestInstance() {
		ValueDescriptor valDesc = new ValueDescriptor(TEST_NAME, TEST_MIN, TEST_MAX, TEST_TYPE, TEST_UOMLABEL,
	...
	static void checkTestData(ValueDescriptor valueDescriptor, String valDescId) {
		...
		assertEquals("ValueDescriptor name does not match saved name", TEST_NAME, valueDescriptor.getName());
		assertEquals("ValueDescriptor min does not match saved min", TEST_MIN, valueDescriptor.getMin());
		assertEquals("ValueDescriptor max does not match saved max", TEST_MAX, valueDescriptor.getMax());
		assertEquals("ValueDescriptor type does not match saved type", TEST_TYPE, valueDescriptor.getType());
		assertEquals("ValueDescriptor label does not match saved label", TEST_UOMLABEL, valueDescriptor.getUomLabel());
		...
		assertEquals("ValueDescriptor origin does not match saved origin", TEST_ORIGIN, valueDescriptor.getOrigin());
