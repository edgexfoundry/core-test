...
public interface ServiceData {
	...
	static void checkTestData(DeviceService s, String id) {
		...
		assertEquals("Device service origin does not match expected", CommonData.TEST_ORIGIN, s.getOrigin());
		assertEquals("Device service admin state does not match expected", TEST_ADMIN, s.getAdminState());
		assertEquals("Device service description does not match expected", TEST_DESCRIPTION, s.getDescription());
		assertEquals("Device service last connected does not match expected", TEST_LAST_CONNECTED,
				s.getLastConnected());
		assertEquals("Device service last reported does not match expected", TEST_LAST_REPORTED, s.getLastReported());
		assertEquals("Device service operating state does not match expected", TEST_OP, s.getOperatingState());
