...
public interface ReportData {
	...
	static void checkTestData(DeviceReport r, String id) {
		...
		assertEquals("Device report origin does not match expected", CommonData.TEST_ORIGIN, r.getOrigin());
		assertEquals("Device report device does not match expected", DeviceData.TEST_NAME, r.getDevice());
		assertEquals("Device report schedule event does not match expected", TEST_SCHEDULE_EVENT_NAME, r.getEvent());
		assertArrayEquals("Device report expected does not match expected", TEST_EXPECTED, r.getExpected());
